    ...
    <div v-if="meetings.length > 0">
        <table>
            ...
            <tr v-for="meeting in meetings" :key="meeting.name">
                ...
                <td>
                    <button @click="deleteParticipant(meeting.name)" v-if="participantAdded(meeting.participants)">
                        Wypisz się
                    </button>
                    <button @click="addNewParticipant(meeting.name)" v-if="!participantAdded(meeting.participants)">
                        Zapisz się
                    </button>
                    <button @click="deleteMeeting(meeting.name)" v-if="meeting.participants.length ===0">Usuń puste
                        spotkanie
                    </button>
                </td>
            </tr>
